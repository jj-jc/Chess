...
namespace ETSIDI
{
	...
	{
		if (w < 0)
			w = (float)_texture.width;
		if (h < 0)
			h = (float)_texture.height;
		_width = w;
		_height = h;
	}
	inline double Sprite::getAngle() const { return _angle; }
	inline void Sprite::setPos(float x, float y)
	{
		pos.x = x;
		pos.y = y;
	}
	inline void Sprite::setCenter(float cx, float cy)
	{
		anchor.x = cx;
		anchor.y = cy;
	}
	inline void Sprite::flip(bool h, bool v)
	{
		horizontal_flip = h;
		vertical_flip = v;
	}
