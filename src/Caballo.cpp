...
bool Caballo::Colision_Caballo(int x, int y) {
	if ((Tablero::posiciones[x-1][y-1] == 1)|| (Tablero::posiciones[x - 1][y - 1] ==2)) {
		if ((Tablero::posiciones[x - 1][y - 1]) != (Tablero::posiciones[getPos().x-1][getPos().y-1])) {
...
{
	glColor3ub(165 * (int)Caballo::getColor()+50, 165 * (int)Caballo::getColor()+50, 165 * (int)Caballo::getColor()+50);
	Base(6.0);
	glPushMatrix();
